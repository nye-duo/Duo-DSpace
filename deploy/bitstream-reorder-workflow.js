...
(function ($) {
    $(document).ready(function() {
        var bitstreamTable = $("table#aspect_xmlworkflow_WorkflowTransformer_table_editItemBitstreams");
    ...
    function renderRowMovement(row, moveUp) {
        ...
        // aspect_xmlworkflow_WorkflowTransformer_cell_bitstream_order_
        var displayedOrder = row.find('td[id^="aspect_xmlworkflow_WorkflowTransformer_cell_bitstream_order_"]');
        displayedOrder.html(newOrder);
    }
    ...
    function isBundleRow(row){
        ...
        return row.attr("id").indexOf("aspect_xmlworkflow_WorkflowTransformer_row_bundle_head_") == 0;
    }
