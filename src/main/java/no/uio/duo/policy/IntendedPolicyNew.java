...
/**
 * Implementation of the IndentedPolicyInterface which can determine the correct policy to be present on a bitstream
 * for an item that is newly submitted to the repository.
 */
public class IntendedPolicyNew implements IntendedPolicyInterface
{
    /**
    ...
    public IntendedPolicy getIntendedPolicies(List<ResourcePolicy> existing, Date embargo)
    {
        ...
        if (embargo == null)    // no embargo
        {
            if (anonRead == null)
            {
                // no embargo date in metadata, and no anonymous read policy - new unbound anonymous read
        ...
        else if (embargo.before(now))   // embargo in the past
        {
            if (anonRead == null)
            {
                // embargo is in the past, and no anonymous read policy - new unbound anonymous read
                intended = new IntendedPolicy(false);
            }
            else
            {
        ...
        else                // embargo is today
        {
            if (anonRead == null)
            {
                // embargo is today, and no anonymous read policy - unbound anonymous read
                intended = new IntendedPolicy(false);
            }
            else
            {
